...
private:
  ...
  void OnWrite(std::shared_ptr<http::response<Body, Fields>> safe_response,
    ...
    logger::LogResponseSent(response_timer_.End(),
...
public:
  ...
  Listener(net::io_context &ioc, const tcp::endpoint &endpoint,
      ...
      : ioc_(ioc)
        // Обработчики асинхронных операций acceptor_ будут вызываться в своём
        // strand
        ,
...
private:
  void DoAccept() {
    acceptor_.async_accept(
        ...
        // С помощью bind_front_handler создаём обработчик, привязанный к методу
        ...
        // bind_front_handler аналогичен namespace ph = std::placeholders;

...
void GameSession::UpdateLoot(int loot_count) {
  for (size_t i = 0; i < loot_count; ++i) {
    ...
    const LootType& loot_type = map_->GetLoots().at(type);
    unsigned value = 1;
    if(loot_type.value.has_value()){
      value = map_->GetLoots().at(type).value.value();
  }

    loot_.emplace_back(std::to_string(auto_loot_counter_++), type, pos, value);
  }
}

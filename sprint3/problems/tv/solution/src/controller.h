...
private:
    ...
    [[nodiscard]] bool ShowInfo(std::istream& input, std::ostream& output) const {
        ...
        if (EnsureNoArgsInInput(INFO_COMMAND, input, output)) {
            ...
            output << "Error: the Info command does not require any arguments" << std::endl;
            return false;
        }
    ...
    [[nodiscard]] bool TurnOn(std::istream& input, std::ostream& output) const {
        ...
        if (EnsureNoArgsInInput(TURN_ON_COMMAND, input, output)) {
            ...
            output << "Error: the TurnOff command does not require any arguments" << std::endl;
            return false;
        }
        return true;
    }
    ...
    [[nodiscard]] bool SelectChannel(std::istream& input, std::ostream& output) const {
        int channel_number;
        if (!(input >> channel_number)) {
            output << "Invalid channel" << std::endl;
            return false;
        }
        try{
            if(tv_.IsTurnedOn()) {
                tv_.SelectChannel(channel_number);
            }
        }

...
std::pair<Token, std::shared_ptr<Player>> Players::AddPlayer(int id, const std::string& name, const Dog* dog, GameSession* session) {
    auto player = std::make_shared<Player>(id, name, dog, session);

    Token token = GenerateToken();
    token_players_[token] = player;

    DogMapId dog_map_id = std::make_pair(dog->GetId(), session->GetMap()->GetId());
...
std::shared_ptr<Player> Players::FindByDogIdAndMapId(int dog_id, std::string map_id){
    try{

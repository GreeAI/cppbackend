...
class Player{
public:
    Player(int id, std::string name, const Dog* dog, const GameSession* session)
        : id_(id)
        , name_(name)
        , dog_(dog)
        , session_(session)
...
class Players {
public:
    using TokenPlayer = std::unordered_map<Token, std::shared_ptr<Player>, util::TaggedHasher<Token>>;
    ...
    using DogMapPlayer = std::unordered_map<DogMapId, std::shared_ptr<Player>, DogMapKeyHasher>;

    std::pair<Token, std::shared_ptr<Player>> AddPlayer(int id, const std::string& name, const Dog* dog, GameSession* session) ;

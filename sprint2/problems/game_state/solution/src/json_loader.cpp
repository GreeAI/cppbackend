...
namespace json_loader
{
    ...
    void LoadingBuildingsIntoMap(model::Map &map, const json::array &buildings_array)
    {
        for (const auto &building_item : buildings_array)
        {
            ...
            // Point
            model::Point pos{static_cast<int>(building_obj.at(Point_X).as_int64()), static_cast<int>(building_obj.at(Point_Y).as_int64())};
            // Size
            model::Size size{static_cast<int>(building_obj.at(Size_W).as_int64()), static_cast<int>(building_obj.at(Size_H).as_int64())};
    ...
    void LoadingOfficesIntoMap(model::Map &map, const json::array &offices_array)
    {
        for (const auto &offices_item : offices_array)
        {
            ...
            // Point
            model::Point pos{static_cast<int>(office_obj.at(Point_X).as_int64()), static_cast<int>(office_obj.at(Point_Y).as_int64())};
            // Offset

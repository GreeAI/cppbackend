  ...
  explicit HandlerApiRequest(Strand api_strand, model::Game &game,
    ...
    if (tick_.has_value()) {
      ticker_ = std::make_shared<Ticker>(
          strand_, FromDouble(*tick_), [this](Milliseconds tick) {
            this->app_.TickTime(tick.count() / 1000);
          });
      std::cout << app_.IsTickSet() << " In Constructor" << std::endl;
      std::cout << *tick_ << " Value in constructor" << std::endl;
      ticker_->Start();
    }
  }
  ...
  StringResponse ApiHandleRequest(Request &&req, model::Game &game) {
    ...
    try {
      ...
      if (StartWithStr(decoded, "/api/v1/game/tick")) {
        ...
        std::cout << app_.IsTickSet() << " In handler" << std::endl;
        if (app_.IsTickSet()) {
          json::object error_code;
          error_code["code"] = "invalidArgument";

...
public:
    ...
    void StartBake(GasCooker& cooker, Handler handler) {
        if (baked_start_time_) {
            throw std::logic_error("Baking already started");
        }

        // Запрещаем повторный вызов StartFry
        baked_start_time_ = Clock::now();
        ...
        cooker.UseBurner([self = shared_from_this(), handler = std::move(handler)] {
            // Запоминаем время фактического начала обжаривания
            self->baked_start_time_ = Clock::now();
            handler();
        });
    }
    ...
    void StopBaking() {
        if (!baked_start_time_) {
            throw std::logic_error("Baked has not started");
        }
        if (baked_end_time_) {
            throw std::logic_error("Baked has already stopped");
        }
        baked_end_time_ = Clock::now();
        // Освобождаем горелку
        gas_cooker_lock_.Unlock();
    }
    ...
    bool IsCooked() const noexcept {
        return baked_start_time_.has_value() && baked_end_time_.has_value();
    }
    ...
    Clock::duration GetBakingDuration() const {
        if (!baked_start_time_ || !baked_end_time_) {
            throw std::logic_error("Sausage has not been cooked");
        }
        return *baked_end_time_ - *baked_start_time_;
    }

private:
    int id_;
    GasCookerLock gas_cooker_lock_;
    std::optional<Clock::time_point> baked_start_time_;
    std::optional<Clock::time_point> baked_end_time_;
};

...
public:
  explicit HandlerApiRequest(Strand api_strand, model::Game &game,
                             std::optional<double> tick, std::optional<std::string> state_file, std::optional<double> tick_state, bool random_spawn)
      : strand_{api_strand}, app_(game, tick.has_value(), state_file, tick_state, random_spawn),
    ...
    if (tick.has_value()) {
      ...
      loot_ticker_ = std::make_shared<app::Ticker>(
...
public:
  explicit RequestHandler(model::Game &game, strct::Args &args,
      ...
      : game_(game), file_handler(args.root),
        api_handler{api_strand, game, args.tick, args.state_file, args.save_tick, args.random_spawn} {}

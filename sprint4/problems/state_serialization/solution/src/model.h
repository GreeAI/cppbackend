...
public:
  ...
  Dog(int id, Name name, Position pos, Speed speed, Direction dir) noexcept
      : id_(id), name_(name), pos_(pos), speed_(speed), dir_(dir), bag_({}) {}
...
private:
  ...
  Bag bag_;
  int score_ = 0;
  size_t bag_capacity_ = 0; // Сделать
};
...
public:
  ...
  void DeleteLootOnMap(std::set<int> collected_loot);

   void LoadLootObjects( std::deque<Loot> loot )  {
    loot_ = std::move(loot);
  }

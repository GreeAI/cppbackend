...
namespace http_handler
{
    ...
    std::unordered_map<std::string, std::string> LogicHandler::ParseTargetArgs(std::string_view req){
        std::unordered_map<std::string, std::string> args;
        
        size_t prev = req.find('?') + 1;;
        size_t next_amper = req.find('&');
        while(next_amper != req.npos){
            std::string arg;
            size_t length = next_amper - prev;
            arg = req.substr(prev, length);

            auto parsed_arg = ParseArg(arg);
            args.emplace(std::move(parsed_arg.first), std::move(parsed_arg.second));

            prev = next_amper + 1;
            next_amper = req.find('&', prev);
        }

        size_t length = next_amper - prev;
        std::string last_arg = std::string(req.substr(prev, length));

...
ParseCommandLine(int argc, const char *const argv[]) {
  ...
  ("tick-period,t", po::value(&tick_per)->value_name("milliseconds"s),"Set tick period")
  ("config-file,c",po::value(&args.config)->value_name("file"s),"Set config file path")
  ("www-root,r", po::value(&args.root)->value_name("dir"s),"Set static files root")
  ("randomize-spawn-points","Put the dog in a random position")
  ("state-file", po::value(&state_file)->value_name("state-file"s),
      "set file path, which saves a game state in procces, and restore it at ""startup")
      ("save-state-period",
                 po::value(&save_tick_state)->value_name("milliseconds"s),
  ...
  if (vm.contains("save-state-period"s)) {
    args.save_state_tick = save_tick_state;
  }
...
template <typename Fn> void RunWorkers(int n, const Fn &fn) {
  n = std::max(1, n);
...
int main(int argc, const char *argv[]) {
  ...
  try {
    ...
    auto db_manager =
        std::make_unique<db_connection::DatabaseManager>(NUM_THREADS, DB_URL);
    ...
    std::shared_ptr<http_handler::RequestHandler> handler =
        std::make_shared<http_handler::RequestHandler>(
            game, args.value(), net::make_strand(ioc), std::move(db_manager));
